...
/**
 * <pre>
 * 详细比对结果标记功能，该功能主要提供一种快速标记无效数据的功能，
...
public class DetailGeneResultMark {
	 ...
	 **/
	public void mark(DetailGeneResultSet result) {
		...
		try {
			...
			watch.stop();
			log.info("标记详细比对结果耗时(ms)==>" + watch.getTime());
		}
	}
	 ...
	 **/
	private void mark(DetailGeneResultSet result, List<String> rIdList) {
	 ...
	 **/
	private void markSames(DetailGeneResultSet result) {
		for (int row = 0; row < result.getRow(); row++) {
			result.invalid(row, row);
